...
sub gromacs_log2hash ($log, $json_filename = undef) {
	...
	if ($log[-1] =~ m/^\h*av\.\h+#atoms communicated per step for force:\h+(.+)/) {
		$data{'av. #atoms communicated per step for force'} = $1;
		splice @log, $#log-1, 2; # remove this line and the blank one behind it
	}
	chomp @log;
#	my @t = @log[($#log-29)..$#log];
#	p @t, array_max => scalar @t;
#	die;
	...
	if ($log[-1] =~ m/^On (\d+) MPI.+(\d+) OpenMP threads/) {
		@{ $data{'REAL CYCLE AND TIME ACCOUNTING'} }{'MPI rank', 'OpenMP'} = ($1, $2);
		pop @log;
	}
	$i = lastidx {/^\h+Total\h+\d+\.\d+\h+/} @log;
	if ($log[$i] =~ m/^\h+Total\h+(\d+)\.(\d+)/) {
		$data{'REAL CYCLE AND TIME ACCOUNTING'}{'Total M-Flops'} = "$1.$2";
		splice @log, $i-1, $#log-$i-1; # remove the "----+" line before it too
	}
	$i = lastidx {index ($_, 'A V E R A G E S') > 0} @log;
	if ($i > 0) {
		my @t = grep {$_ !~ m/#+\h+=+/} @log[($i+1)..$#log];
		...
		foreach (@t) {
			...
			if ((/^\h+\-?\d/) && (scalar @energies > 0)) {
				...
				foreach my $energy (@energies) {
					push @{ $data{AVERAGES}{$energy} }, $line[$e++];
				}
				undef @energies;
			}
		}
		splice @log, $i-1, $#log-$i-1; # and the "== #+" before it
	}
	foreach my $i (reverse 0..$#log) { # remove blank lines at the end
		last if $log[$i] =~ m/\S/;
		pop @log;
	}
	my $z = first { $log[$_] =~ m/^Started\h.+on rank.+\d+$/} 0..$#log;
	foreach my $i (grep {$log[$_] =~ m/^DD\h+step\h+\d+/} reverse $z..$#log) {
		splice @log, $i-1, 2; # remove this line and the blank that comes before it
	}
	foreach my $i (grep {$log[$_] =~ m/^step\h+\d+\h+Turning on/} reverse $z..$#log) {
		splice @log, $i, 1;
	}
	foreach my $i (grep {$log[$_] =~ m/^Current ref_t for group System:\h+\d+\.\d+/} reverse $z..$#log) {
		splice @log, $i-1, 2;
	}
	foreach my $i (grep {index($log[$_],'PLEASE READ AND CITE THE FOLLOWING REFERENCE ') > 0} 0..$#log) {
		my $thanks_line;
		foreach my $j ($i..$#log) {
			if (index($log[$j], ' Thank You ') > 0) {
				$thanks_line = $j;
				last;
			}
		}
	...
	my @colvars_lines = grep { index($_, 'colvars:') == 0 } @log;
	if (scalar @colvars_lines > 0) {
		...
		foreach my $start_idx (@colvars_start_idx) {
			...
			my $name;
			foreach my ($i2, $line) (indexed @colvar) {
...
# ensure that I got the correct number of collective variables
	...
	@log = grep {index($_, 'colvars: ') == -1} @log; # I've already dealt with these, remove to prevent bugs
	$i = first_index {index('qm-opts:', $_) == 0} @log;
	if ($i > 0) {
	...
	foreach my $index (reverse grep { index($log[$_], 'Writing checkpoint') == 0 } 99..$#log) {
		 splice @log, $index, 3;
	}
	my $input_param_i = first_index {index($_, 'Input Parameters:') == 0} @log;
	my $ending_i = first {$log[$_] =~ m/^\S/} $input_param_i+1..$#log;
	foreach my $i ($input_param_i..$ending_i) {
		$log[$i] =~ s/^\h+//;
	...
	splice @log, 0, first_index { index($_, 'GROMACS: ') == 0} @log;
	$i = first_index {index($_, ' Step ') > 0 && index($_, ' Time') > 0} @log;
	my @time_indices = grep {
									$log[$_-1] eq '           Step           Time'
								&&
									$log[$_] =~ m/
									^\h+
									\d+        # Step
								...
								&&
									(
										(index($log[$_+2], 'Energies (kJ/mol)') > 0)
										||
										(index($log[$_+3], 'Energies (kJ/mol)') > 0)
									)
	...
	foreach (@log) {
		...
		if ((/^\h+\-?\d/) && (scalar @energies > 0)) {
			...
			my $e = 0;
			foreach my $energy (@energies) {
				push @{ $data{Energy}{$energy} }, $line[$e++];
			}
			undef @energies;
		}
	}
	...
	$i = first {index($_, 'Running on ') == 0} @log;
	if ($i =~ m/^Running on (\d+) nodes? with total (\d+) cores, (\d+) processing units/) {
		$data{'CPU info'}{nodes} = $1;
		$data{'CPU info'}{cores} = $2;
		$data{'CPU info'}{'processing units'} = $3;
	}
	if ($i =~ m/(\d+) compatible GPUs?$/) {
		$data{'CPU info'}{'compatible GPU'} = $1;
	}
#		splice @log, $i, 1;
	$i = first_index { index($_, 'Hardware detected on host ') == 0 } @log;
	die "Couldn't find Hardware in $log" if $i == -1;
	if ($log[$i] =~ m/^Hardware detected on host (.+):/) {
		$data{Host} = $1;
	} else {
		die "this line failed regex to get the hostname:\n$log[$i]";
	}
